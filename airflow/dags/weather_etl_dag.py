...
# from etl.utils.adapters.openmeteo import WeatherExtract, WeatherTrasnform
...
from etl.weather import openmeteo
...
def weather_etl():
    ...
    def weather_extract(triggering_asset_events):
        ...
        # )
        with openmeteo.WeatherExtract(
            stamp(airport_location_for_weather_filepath, TIMESTAMP),
            airport_location_filepath,
            schema_filepath=SCHEMA_FILEPATH,
            weather_url=WEATHER_URL,
        ) as weather_extractor:
            weather_extractor.execute(stamp(bronze_weather_filepath, TIMESTAMP))

        return TIMESTAMP

    @task(outlets=[weather_asset])
    def weather_transform(TIMESTAMP: str):
        weather_transformer = openmeteo.WeatherTransform(
            stamp(bronze_weather_filepath, TIMESTAMP),
            stamp(airport_location_for_weather_filepath, TIMESTAMP),
            schema_filepath=SCHEMA_FILEPATH,
        )
        weather_transformer.execute(stamp(silver_weather_filepath, TIMESTAMP))
